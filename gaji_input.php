 ...
 include("conf/format_rupiah.php");
	...
	if(isset($_GET['kry'])) {
		...
		while($data = mysqli_fetch_array($ress)) {
			...
			//cari lembur
			$sqllem = "SELECT * FROM lembur WHERE lembur_bl='$bl' AND lembur_th='$th'";
			...
			$ttllembur = $jamlembur*$galem;
						...
						//cari tunjangan
			$sqltjg = "SELECT * FROM tunjangan WHERE tjg_bl='$bl' AND tjg_th='$th'";
			...
			//cari potongan
			$sqlpot = "SELECT * FROM potongan WHERE pot_bl='$bl' AND pot_th='$th'";
	...
	if(isset($_POST['update'])){
		...
		if($rowscek>0){
			...
			$sql  = "INSERT INTO gaji_karyawan(gaj_no,karyawan_id,abs_id,gaj_stt,gaj_pay,gaj_ket)VALUES('$num','$id','$abs','$stt','$via','$ket')";
			$ress = mysqli_query($conn, $sql);
			if($ress){
				echo "<script>alert('Pengajuan berhasil!');</script>";
				echo "<script type='text/javascript'> document.location = 'gaji_lihat.php?abs=$abs'; </script>";
