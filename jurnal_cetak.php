<?php
	...
	if($bln=="all"){
		...
		$sql = "SELECT jurnal.*, akun.* FROM jurnal, akun 
				WHERE jurnal.akun_kode=akun.akun_kode ORDER BY jurnal.jurnal_tgl ASC";
		$ress = mysqli_query($conn, $sql);

		$sqldeb = "SELECT jurnal.*, akun.* FROM jurnal, akun 
				WHERE jurnal.akun_kode=akun.akun_kode AND akun.akun_jenis='Debet'
			...
			<br />	
				...
				<thead>
					<tr>
						...
						<th width="8%"><center>Kredit</center></th>
						<th width="8%"><center>Debet</center></th>
					</tr>
				</thead>
				<tbody>
				<?php
					...
					while($data=mysqli_fetch_array($ress)){
						...
						echo '<td class="text-center">';
							if($data['akun_jenis']=="Kredit"){
								echo format_rupiah($data['jurnal_jml']);
							}else{
								echo "-";
							}
						echo '</td>';
						echo '<td class="text-center">';
							if($data['akun_jenis']=="Debet"){
								echo format_rupiah($data['jurnal_jml']);
							}else{
								echo "-";
							}
				...
				<tfoot>
					<tr>
						...
						<td class="text-center"><b>Total Kredit <br/> <?php echo format_rupiah($kre);?></b></td>
						<td class="text-center"><b>Total Debet <br/> <?php echo format_rupiah($deb);?></b></td>				
				</tfoot>
